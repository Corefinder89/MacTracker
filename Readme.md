...
In this application I have taken a dump of `NMAP` output in an xml format using the parameter `-oX`. The `NMAP` command looks like this `nmap -sP -n -oX - 192.168.2.1/24 | tee nmap.xml`. You can use the parameter `-oN` to output the desired result in a text format as well.  
